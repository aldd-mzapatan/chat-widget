...
function extractCodeBlocks(text, blocks) {
  return text.replace(/```(\w*)\n?([\s\S]*?)```/g, (_, lang, code) => {
    ...
    blocks.push(`<pre class="cb-code-block"><code${langAttr}>${escapeHtml(code.trim())}</code></pre>`);
...
var ChatBubble = class {
  ...
  async _loadHistory() {
    ...
    for (const msg of oldHistory) {
      ...
      if (msg.role === "user") {
        ...
        this._addBotMessage(msg.text, msg.isError, dateObj, false, false);
      }
    }
  ...
  async _loadEmojiPicker() {
    if (this._emojiPickerLoaded)
      return;
    try {
      await import("./emoji-picker-element-ETT3QMK4.js");
  ...
  _bindEvents() {
    ...
    const handleEmojiPickerKeydown = (e) => {
      ...
      if (e.key === "Tab") {
        ...
        if (e.shiftKey) {
          if (document.activeElement === first || document.activeElement === this.emojiPicker) {
            e.preventDefault();
            last.focus();
          }
        } else {
    ...
    document.addEventListener("click", (e) => {
      if (!this.emojiPicker.classList.contains("cb-visible"))
        return;
      ...
      if (this.emojiPickerEl) {
        ...
        if (path.includes(this.emojiPickerEl) || path.includes(this.emojiPicker)) {
          return;
        }
      }
      this.emojiPicker.classList.remove("cb-visible");
  ...
  _initDraggable() {
    ...
    this.header.addEventListener("pointerdown", (e) => {
      if (e.target.closest(".cb-toggle-size-btn") || e.target.closest("#cb-resize-handle") || window.innerWidth <= 480) {
        return;
      }
  ...
  _addBotMessage(text, isError = false, dateObj = /* @__PURE__ */ new Date(), save = true, isAnimated = true) {
